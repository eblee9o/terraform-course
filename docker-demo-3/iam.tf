########################################
# iam.tf  —  생성/참조 분기(가드) 완전본
########################################

########################################
# Variables (원하시면 vars.tf로 옮겨도 됩니다)
########################################
variable "create_iam" {
  description = "이 워크스페이스에서 IAM 리소스를 새로 생성할지 여부"
  type        = bool
  default     = false
}

variable "create_service_roles" {
  description = "ecs-service-role / ecs-consul-server-role을 이 워크스페이스에서 생성할지 여부"
  type        = bool
  default     = false
}

variable "ecs_ec2_role_name" {
  description = "이미 존재(또는 새로 만들) EC2 인스턴스용 IAM Role/InstanceProfile의 이름"
  type        = string
  default     = "ecs-ec2-role"
}

variable "ecs_service_role_name" {
  description = "이미 존재(또는 새로 만들) ECS 서비스용 IAM Role 이름"
  type        = string
  default     = "ecs-service-role"
}

variable "ecs_consul_server_role_name" {
  description = "이미 존재(또는 새로 만들) Consul 서버용 IAM Role 이름"
  type        = string
  default     = "ecs-consul-server-role"
}

########################################
# (A) 생성 경로 — create_iam / create_service_roles 가 true일 때만 생성
########################################
